...
- **go-cqrs-lite v4.9-era API (bumped 2026-09-07)** — all 14 consumed submodules ride their latest published tags (decider v4.5.0, event v4.9.0, storage/bbolt v4.1.0, ...); `codec/v4`/`flightrecorder/v4` were split upstream into standalone `go-codec`/`go-flightrecorder` modules. The Store uses the `LoadRef`/`ExecuteRef` forms with `id.NewStreamRef` (the pair-form `Load`/`Execute` is deprecated upstream, removed in v5). On-disk journals are compatible across this bump, and the claim is now **machine-pinned, not inspected**: `golden_journal_test.go` loads a frozen pre-bump-format journal (`testdata/golden-journal.bbolt`, provenance + regeneration recipe in `testdata/golden-journal.PROVENANCE.md`) and asserts fold, payload decode, raw CBOR envelope (`serializableEvent` key set, `schema_version=1`, `encoding=cbor`), payload JSON tags, plus a fuzz target over the decode path. Wire facts: bucket names and the `serializableEvent` struct/tags are byte-identical between bbolt v4.0.0 and v4.1.0; envelopes are CBOR with a JSON-fallback read path; events stamp their payload codec (`DefaultCodec` = CBOR) so `DecodePayloadAuto` handles mixed streams; CBOR roundtrips `time.Time` to a different `time.Location` of the same instant — compare with `.Equal`, never `==`
- **Deterministic replay** — INDEX timestamps derive from the newest row update time (`indexStamp`), never `time.Now()`, so a pass and a replay of the same journal produce identical files. Row "Updated" times come from `ViewState.UpdatedAt()`: the review time when the current capture was reviewed, else the capture time — pass-time refresh and Replay share the helper
...
- **`GOEXPERIMENT=jsonv2` required in nix builds** — go-cqrs-lite imports `encoding/json/v2`, which the sandboxed Go toolchain excludes without the experiment; without it buildGoModule "succeeds" with an EMPTY output. In local shells the experiment arrives as an exported `GOEXPERIMENT=jsonv2` environment variable (the `~/.config/go/env` file does NOT set it; verified 2026-09-22), so plain `go build` inside such shells is jsonv2-regime — an `encoding/json/v2` import therefore compiles locally and only fails in CI's `no-jsonv2` job
...
- **Website-fleet pipeline (activated 2026-09-19)** — `scripts/shoot-sites.sh` captures all 17 site homes (desktop+mobile, `--blink-settings=lazyImageLoadingEnabled=false` is MANDATORY — lazy images otherwise screenshot as blank cards) with blank-capture (ffmpeg luma spread) and error-page (dump-dom grep) verify gates; review config lives durably at `~/.config/visionreviewd/websites.json` (repo copy: `docs/activation/visionreviewd-websites-17.json`). Keep capture → verify → review serialized; fleet ops (custom-domain attach steps, cert/DNS incident history, hourly `site-monitor.timer`) live in `docs/activation/site-fleet-ops.md`
- **llama-server must start from direct local paths** — the `-hf <repo>` flag hangs ~10 min (HF resolution + download onto the degraded `/data` NVMe, 5-8 MB/s measured; root NVMe is healthy at 3-4 GB/s, see `docs/activation/site-fleet-ops.md` §1.3 for SMART/bench commands). Working invocation is in `docs/activation/README.md`; health gate `http://127.0.0.1:8390/health`
- **Capture Chrome must be the nix-store chromium with `--no-sandbox`** — path pinned inside `scripts/shoot-sites.sh` (`VISION_CHROME` overrides); headless without `--no-sandbox` fails in this user namespace
