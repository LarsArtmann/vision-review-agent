...
luma_spread() {
	...
	local img="$1" out
	out=$(
		$FFMPEG -hide_banner -i "$img" -vf "signalstats,metadata=print:key=lavfi.signalstats.YMAX:file=-" -f null - 2>/dev/null
...
verify_or_die() {
	...
	if [ ! -s "$img" ]; then
		echo "  VERIFY-FAIL $proj/$view: missing/empty"
		return 1
	fi
	size=$(stat -c%s "$img")
	if [ "$size" -lt "$MIN_BYTES" ]; then
		echo "  VERIFY-FAIL $proj/$view: only ${size}B"
		return 1
	fi
	...
	if [ "$spread" -lt 0 ]; then
		...
		echo "  VERIFY-FAIL $proj/$view: blank (luma spread $spread < $VARIANCE_MIN)"
		return 1
	else
...
for s in "${SITES[@]}"; do
	...
	if dom_has_error "$url"; then
		echo "  VERIFY-FAIL $proj: error page detected in DOM"
		fail=$((fail + 1))
		continue
	fi
