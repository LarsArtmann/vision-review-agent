...
The BuildFlow pre-commit hook runs `go-structure-linter`, `library-policy`, `todo-check`, and `nix-fmt` against the *entire project* on every commit. This means:
...
This design defeats the purpose of pre-commit hooks, which should validate *the commit being made*, not the entire project's accumulated debt. The user has to add `--no-verify` to every commit, which trains them to ignore the hook entirely.
...
**What is the intended behavior?** Should pre-commit only block commits that introduce *new* problems, or should it enforce a continuously clean project state?
