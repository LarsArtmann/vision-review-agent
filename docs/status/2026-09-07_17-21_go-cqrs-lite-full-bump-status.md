...
3. **Lint baseline — inferred, not measured.** Baseline covered build+test only.
   ...
   I never touched config.go), but I never ran pre-bump lint to _prove_ the
...
2. **A mid-task state was committed unverified.** The auto-commit daemon landed
   ...
   before editing store.go), but I never _proved_ that exact commit green.
...
_Near-term = do before the next release; ROADMAP fuel = schedule, don't commit._
