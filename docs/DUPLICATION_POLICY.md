...
| `examples/openai/main.go` and `examples/structured/main.go` OpenAI provider + model construction                              | Example boilerplate                    | Both examples show readers how to wire up the OpenAI provider. Self-contained examples are the point — extracting into `cli` couples the examples to internal helpers and breaks copy-paste usability. The `cmd/vision` tool already factors this out where it matters. |
| `examples/openai/main.go` and `examples/structured/main.go` `cli.NewAgent` + `vision.LoadImageFromFile` calls                 | Example boilerplate                    | Same reasoning as above; each example must show a runnable flow end-to-end.                                                                                                                                                                                             |
